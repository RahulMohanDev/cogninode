...
const { renderBody, getModel, ModelDot, MODELS, fmt, calculateCostUsd, estimateCostUsd, formatCost, formatEstimate, loadCustomModels } = window;

// Combined model list — built-ins + user-added custom models.
const getAllModels = () => [...MODELS, ...loadCustomModels()];

// ------------------------------------------------------------------
// Cost pill — colour-coded estimate shown in the composer footer
// ------------------------------------------------------------------
const CostPill = ({ costUsd, model }) => {
  const cls =
    costUsd === 0      ? "cp-free" :
    costUsd <  0.005   ? "cp-low"  :
    costUsd <= 0.02    ? "cp-mid"  : "cp-high";
  return (
    <span className={`cost-pill ${cls}`} title={`Estimated cost on ${model?.name || "this model"}`}>
      {formatEstimate(costUsd)}
    </span>
  );
};
// Back-compat: chat-app may still import CreditPill. Same shape.
const CreditPill = CostPill;
      ...
      {!reflecting && isAssistant && model && (
        <div className="m-foot">
          <span className="credits">
            ...
            {typeof msg.costUsd === "number" ? formatCost(msg.costUsd)
              : typeof msg.credits === "number"
                ? formatCost(
                    // Seed data uses the old INR-derived credit system;
                    // convert back to a rough USD figure so the demo
                    // shows plausible numbers (credits × ₹0.08 / 84 / 2× margin).
                    msg.credits * 0.08 / 84 / 2
                  )
              : "—"}
...
// ------------------------------------------------------------------
const ModelPicker = ({ currentId, onPick, onClose, onAddCustom }) => {
  ...
  <>
    ...
    <div className="model-pop">
      {allModels.map(m => {
        const cost = calculateCostUsd(1200, 600, m);
        const isFree = cost === 0;
        return (
          <div
            key={m.id}
          ...
          >
            ...
            <div className="mp-meta">
              <span className="mp-name">{m.name}{m.custom && <span className="mp-custom-tag"> · custom</span>}</span>
              <span className="mp-tag">{m.vendor.toLowerCase()} · {m.tag}</span>
            </div>
            <span className={`mp-cred ${isFree ? "is-free" : ""}`}>
              {isFree ? "free" : formatCost(cost)}
            </span>
          </div>
        );
      })}
      <div className="mp-sep"></div>
      <div className="mp-row mp-add" onClick={() => { onClose(); onAddCustom?.(); }}>
...
// Composer
// ------------------------------------------------------------------
const Composer = ({ value, onChange, onSend, quote, onClearQuote, files, onAddFile, onRemoveFile, modelId, onChangeModel, disabled, pathMessages, onOpenSettings }) => {
  const [uploadOpen, setUploadOpen] = React.useState(false);
  const [modelOpen, setModelOpen] = React.useState(false);
  const [estCost, setEstCost] = React.useState(0);
  const taRef = React.useRef(null);

  // Recalculate cost estimate on text / model / path change. Debounced 150ms
  // so the pill doesn't repaint on every keystroke for long messages.
  React.useEffect(() => {
    const model = getModel(modelId) || getAllModels().find(m => m.id === modelId);
    const t = setTimeout(() => {
      setEstCost(estimateCostUsd(value, pathMessages || [], model));
    }, 150);
  ...
  const model = getModel(modelId) || getAllModels().find(m => m.id === modelId) || MODELS[0];
    ...
    <div className="composer">
      ...
      {estCost > 0.015 && (
        <div className="branch-hint">
          ...
          Branch to reset path — save ~{formatCost(estCost * 0.55)}
        </div>
      )}
      ...
      <div className="composer-bottom">
        <div className="left">
          ...
          <button className="model-pick" onClick={() => setModelOpen(v => !v)}>
            ...
            {modelOpen && <ModelPicker currentId={modelId} onPick={onChangeModel} onClose={() => setModelOpen(false)} onAddCustom={onOpenSettings}/>}
          </button>

          <CostPill costUsd={estCost} model={model} />
        </div>
...
Object.assign(window, { SelectionPopup, Message, UploadMenu, ModelPicker, Composer, CostPill, CreditPill });
